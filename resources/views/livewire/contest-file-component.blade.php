<div class="container">
    <div>
        ...
        <h1>Archivos concurso {{ $contest->title }}</h1>
                ...
                <tbody>
                    @if (count($files) > 0)
                        @foreach ($files as $file)
                            <tr>
                                <td>{{ $file->title }}</td>
                                <td class="text-center">{{ $file->file_url }}</td>
                                ...
                                <td class="text-center">
                                    <button class="btn btn-sm btn-info" wire:click='setEditMode({{ $file->id }})'>
                                        <i class="fas fa-pen"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteModal"
                                        wire:click="$set('file_id', {{ $file->id }})">
